...
- `data_length`: the length of the entire sprite data, including header and this
  field. This may sometimes be `0`, in which case the size will need to be
  manually determined by adding to `pixel_data_offset` the last offset `&
...
(i.e. a mask of `0xf800`). Palette sets apply to all sprites for a given sprite
file. `num_palette_sets` and `palette_offset` must be ignored if `bpp >= 16`.

If encryption is enabled, it is applied to the pixel data only, starting after
the list of offsets and lengths if pixel data is compressed (see below). The
encryption is XORing the byte `0x53` over each byte of an individual sprite's
pixel data as stored, whether compressed or not. Encryption is not available for
`bpp` >= `16`.
...
packed into a bitstream read from the least significant bit of each byte, with
each element being having the bit length indiecated by the `bpp` field. When
reading the element, shift each bit into the least significant end of your
holding variable. There is no stride to account for. For `bpp` >= `16`, palettes
are not used and instead each pixel is read as 16-bit RGB565 directly from the
pixel data. Sprites are stored sequentially, and subimages are composed of
sprites in left to right, top to bottom order. Subimages are stored sequentially
as well.
...
bytes, and offsets are relative to `pixel_data_offset` and in ascending order.
Lengths are the lengths of the compressed data, not the decompressed data. If
the top bit of the offset is set, that means the data is not compressed and
should be read as is (of course, unset the top bit first), and the length
indicates the length of the data. The start of compressed data should be 32-bit
...
byte/word. If the control byte/word is exactly `0`, decompression is finished
and any trailing compressed data ignored. If the top bit of the byte/word is
set, the next `n` bytes/words are literals. `n` is the control byte `& 0x7f` for
bytewise compression, and the control word `& 0x0fffffff` for wordwise
compression. If the top bit of the control byte/word is not set, then the next
byte/word is read and `n` bytes/words are repeated with this value, where `n` is
the control byte/word. Note for wordwise compression, bits 28-30 of the control
word are ignored. If the top bit is set but `n` is `0`, then this is a no-op.
